...
public class TicketService {
	...
	private void actualizarStock(Set<Line> lines) {
		for (Line line : lines) {
			...
			var productDB = this.productRepository.findById(product.getId());
	...
	private Ticket createTicket(Ticket ticket) {
		...
		ticketToSave.setClient(ticket.getClient());
	...
	private Boolean existsStock(Set<Line> lines) {
		for (Line line : lines) {
			...
			if (line.getQuantity() > opProduct.get().getStock()) {
			    return false;
			}
		}
		return false;
	...
	private Boolean existsClient(Client client) {
		var opClient = this.clientRepository.findById(client.getById());
		return !opClient.isEmpty();
	}
}
